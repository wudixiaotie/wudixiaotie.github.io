...
当一个载体已经没有空闲的内存块的时候，它会被从载体池中拿出。所有满负荷的载体都会被传递给它的雇主，好让其通过延迟释放功能来释放没用的内存。这种通过雇主来释放载体的方式，让底层分配和释放内存变得很简单，不用考虑多线程的情况。在NUMA架构的时候，我们也不会让来自不同NUMA节点的载体混在一起。

简短来说：

* 内存分配器创建的载体归这个内存分配器所有
* 一个满负荷的载体永远都是由它的“所有者”来释放
* 这种所属关系永远不会改变
